...
_interactionKey = if (count (actionKeys "User10") isEqualTo 0) then {15 } else {(actionKeys "User10") select 0};
...
switch (_code) do {
    ...
    */
    ...
    //Kod 1 Shift+1
    case 2: {
        if(_shift && playerSide isEqualTo west && vehicle player != player) then {
            hint "Kod 1";
    ...
    case 3: {
        if(_shift && playerSide isEqualTo west && vehicle player != player) then {
            hint "Kod 2";
            if( typeOf(vehicle player) IN ["ivory_m3_pd_60","kmc_wrx_pd_17","ivory_isf_marked","ivory_isf_unmarked","ivory_isf_slicktop","ivory_m3_marked","ivory_m3_unmarked","ivory_m3_slicktop","ivory_wrx_marked","ivory_wrx_unmarked","ivory_wrx_slicktop"] ) then {
  				vehicle player animate ["ani_siren", 0];
	...
	if(_ctrlKey) then {_handled = true;};
		...
		//if !(license_civ_rebel) exitWith { hintSilent "Nie nalezy do Al-Kaid'y!"; };
		if(_ctrlKey && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,west]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
    ...
    */
    ...
    //F3 PANIC BUTTON
    case 61: {
        if(playerSide isEqualTo west && {life_inv_panic > 0}) then {
    ...
    case 34: {
        ...
        if (_shift && playerSide isEqualTo west && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
            if ((animationState cursorObject) != "Incapacitated" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable ["restrained",false]) && !life_istazed && !life_isknocked) then {
                [cursorObject] spawn life_fnc_knockoutAction;
            };
        };
    ...
    //F Key
    case 33: {
        if (_ctrlKey && playerSide in [west,independent] && {vehicle player != player} && {!life_siren_active} && {((driver vehicle player) == player)}) then {
            [] spawn {
    ...
    case 25: {
        if (!_alt && _shift && playerSide isEqualTo west) then {
            [] call life_fnc_ChangePolice;
        };
        if (!_shift && _alt && playerSide isEqualTo west) then {
            [] call life_fnc_ChangeEMS;
        };
        if (_shift && playerSide isEqualTo independent) then {
            [] call life_fnc_ChangeEMS;
        };
        if (!_shift && _alt && playerSide isEqualTo independent) then {
            [] call life_fnc_ChangePolice;
        };
    };
