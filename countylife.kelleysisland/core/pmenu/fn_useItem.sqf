...
switch (true) do {
    ...
    case (_item isEqualTo "stretcher"): {
        if ([false,_item,1] call life_fnc_handleInv) then {
            [] spawn life_fnc_stretcher;
            closeDialog 0;
        };
    };
    ...
    case (_item isEqualTo "cocaine_processed"): {
        if (([false,_item,1] call life_fnc_handleInv)) then {
            ...
            [] spawn {
                life_redgull_effect = time;
                player enableFatigue false;
                waitUntil {!alive player || ((time - life_redgull_effect) > (15 * 60))};
                player enableFatigue true;
            };
        };
    };

    case (_item in ["beer","whiskey","moonshine"]): {
        if (([false,_item,1] call life_fnc_handleInv)) then {
            [] spawn life_fnc_alkohol;
        }; 
    };
