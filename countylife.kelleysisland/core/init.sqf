...
/* TASK FORCE RADIO SZTUFF UND OTHER SZIT*/
player setVariable ["tf_receivingDistanceMultiplicator", 3];
player setVariable ["tf_sendingDistanceMultiplicator", 3];
...
Dvid_TFEnabled = true;
Dvid_onTsServer = "CountyLife.PL" == (call TFAR_fnc_getTeamSpeakServerName); ///////////////////////Edit This line (the channel at the top of your TS (where you right click and "Edit Virtual Server"), yes, get the name of that goes here)
...
[] spawn {
	...
	while {true} do {	
					...
					if (!(_isadmin)) then {
						titleText ["Włącz plugin TFAR na TS! || TS3 -> Tools -> Options -> Addons -> TFAR", "BLACK"];
					...
					_onTsServer = "CountyLife.PL" == (call TFAR_fnc_getTeamSpeakServerName); //////////////////////Edit too pls, or dont but it wont work if you dont
					if (!(_onTsServer)) then {
					if (!(_isadmin)) then {
						titleText ["Wejdź na serwerowy TS! Adres: ts.countylife.pl", "BLACK"];
						...
						if (!(_isadmin)) then {
							titleText ["Wszystko gotowe. Witamy!","BLACK IN"];
					...
					if (!(_onChannel)) then {
					if (!(_isadmin)) then {
						titleText ["Przeładuj plugin TFAR na TS by dołączyć na kanał! || Tools -> Options -> Addons -> Reload All", "BLACK"];
					...
					if ((_TFenabled) && (!(Dvid_TFEnabled))) then {
						titleText ["Wszystko gotowe. Witamy z powrotem!","BLACK IN"];
						Dvid_TFEnabled = true;
						if (!(player getvariable "taskfr")) then {
							player setvariable ["taskfr",true,true];
						};
