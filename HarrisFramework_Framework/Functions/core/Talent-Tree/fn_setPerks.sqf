...
HRP_fnc_setPerks = {
    ...
    private _iterate = {
        {
            ...
            if (_first isEqualType []) then {
                private _add = [_oldPerks, _first param[0]] call HRP_fnc_hasPerk;
                if (_add) then {
                    _first set [1, true];
                ...
                private _add = [_oldPerks, _first] call HRP_fnc_hasPerk;
                if (_add) then {
                    _x set [1, true];
    ...
    private _newPerks = [] call HRP_fnc_generateBasePerks;
