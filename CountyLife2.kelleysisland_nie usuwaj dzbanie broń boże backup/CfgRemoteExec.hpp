...
class CfgRemoteExec {
    ...
    allowedTargets = 2;
        ...
        F(life_fnc_tieing,CLIENT) //ziptie
        F(life_fnc_tieingb,CLIENT) //worek
        F(life_fnc_removeWeaponActionciv,CLIENT) //Patdown
        F(life_fnc_removeWeaponAction,CLIENT) //StripDown
        F(life_fnc_bandagesA,CLIENT)
        F(life_fnc_morphineA,CLIENT)
        F(life_fnc_medkitA,CLIENT)
        F(life_fnc_ringPlayer,CLIENT)//PHONE
        F(life_fnc_busyAnswer,CLIENT)
        F(life_fnc_revokeDriverLicense,CLIENT)
        F(life_fnc_revokeGunLicense,CLIENT)
        F(life_fnc_zgon,CLIENT)
