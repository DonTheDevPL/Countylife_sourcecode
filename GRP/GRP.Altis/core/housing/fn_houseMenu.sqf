...
if (!(_curTarget in life_vehicles) || isNil {_curTarget getVariable "house_owner"}) then {
    ...
    if (isClass (missionConfigFile >> "Garages" >> worldName >> (typeOf _curTarget))) then {
        ...
        /*
            //Fabryki
        */
        if(typeOf cursorObject in ["Land_Addon_05_F","Land_dp_smallFactory_F","Land_i_Shed_Ind_F","Land_MilOffices_V1_F"]) exitWith {
            _Btn1 ctrlSetText localize "STR_pInAct_SellHouse";
            _Btn1 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_sellHouse; closeDialog 0;";
            _Btn1 ctrlShow true;

            _Btn2 ctrlSetText "Zacznij przetarg";
            _Btn2 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_selectFactory; closeDialog 0;";
            _Btn2 ctrlShow true;
        };
        _Btn1 ctrlSetText localize "STR_pInAct_SellHouse";
        _Btn1 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_sellHouse; closeDialog 0;";
