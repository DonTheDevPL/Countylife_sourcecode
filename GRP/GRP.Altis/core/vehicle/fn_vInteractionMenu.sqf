...
if (playerSide isEqualTo west) then {
    ...
    if (_curTarget isKindOf "Ship") then {
        ...
        if (local _curTarget && crew _curTarget isEqualTo []) then { _Btn6 ctrlEnable true;} else {_Btn6 ctrlEnable false};
    } else {
        if (!isNil "_id") then {
    ...
    */
    ...
    if (_curTarget isKindOf "Ship") then {
        ...
        if (local _curTarget && crew _curTarget isEqualTo []) then { _Btn2 ctrlEnable true;} else {_Btn2 ctrlEnable false};
    } else {
        if (!isNil "_id") then {
            if !(_id in getDLCs 1) then {
    ...
    */
    _Btn4 ctrlSetText "STR_vInAct_PullOut";
