...
switch (_code) do {
    ...
    */
    ...
    case 46	: {
            if(life_barrier_active && playerSide in [west,independent]) then {
    ...
    case 19: {
        ...
        if (_shift && playerSide isEqualTo west && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,independent])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
            if(cursorObject getVariable ["playerSurrender",false] || (animationState cursorObject) == "Incapacitated" || (cursorObject getVariable ["istazed",false])) then {
    ...
    */
    ...
    case 24: {
        if (_shift) then {
            if !(soundVolume isEqualTo 1) then {
                1 fadeSound 1;
                hint localize "STR_MISC_soundnormal";
                life_fadeSound = false;
                [] call life_fnc_hudUpdate;
            } else {
                1 fadeSound 0.1;
                hint localize "STR_MISC_soundfade";
                life_fadeSound = true;
                [] call life_fnc_hudUpdate;
            };
        };
    ...
    case 207: {
        if(playerSide isEqualTo west) then {
            [(call TFAR_fnc_activeSwRadio), 1, 1033] call TFAR_fnc_SetChannelFrequency;
            hint "Jesteś na zastrzeżonej częstotliwości";
        };
        if(playerSide isEqualTo independent) then {
            [(call TFAR_fnc_activeSwRadio), 1, 1032] call TFAR_fnc_SetChannelFrequency;
            hint "Jesteś na zastrzeżonej częstotliwości";
        };
    };
    //U Key
            ...
            if (_veh isKindOf "House_F" && {playerSide isEqualTo civilian}) then {
                if (_veh in life_vehicles && {player distance _veh < 20}) then {
                    ...
                    if (_locked isEqualTo 0) then {
                        ...
                        hint localize "STR_House_Door_Lock";
                    } else {
                        _veh setVariable [format ["bis_disabled_Door_%1",_door],0,true];
                        _veh animateSource [format ["Door_%1_source", _door], 1];
                        hint localize "STR_House_Door_Unlock";
                    };
                };
            } else {
                _locked = locked _veh;
