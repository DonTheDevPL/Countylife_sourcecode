...
switch (playerSide) do {
	case west: {
		...
		//PatDown
		_Btn6 ctrlSetText localize "STR_pInAct_PatDown";
		_Btn6 buttonSetAction "[life_pInact_curTarget,1] spawn life_fnc_patdown; closeDialog 0;";

		//StripDow
		_Btn7 ctrlSetText localize "STR_pInAct_StripDown";
		_Btn7 buttonSetAction "[life_pInact_curTarget,0] spawn life_fnc_patdown; closeDialog 0;";

		//StripDown
		_Btn8 ctrlSetText localize "STR_pInAct_PutInCar";
		_Btn8 buttonSetAction "[life_pInact_curTarget,2] call life_fnc_putInCar; closeDialog 0;";

		//Jail
		_Btn9 ctrlSetText localize "STR_pInAct_Arrest";
		_Btn9 buttonSetAction "[life_pInact_curTarget] call life_fnc_showArrestDialog;";
		_Btn9 ctrlEnable false;

		if (FETCH_CONST(life_coplevel) < _seizeRank) then {_Btn8 ctrlEnable false;};

		{
			if ((player distance (getMarkerPos _x) <30)) exitWith { _Btn9 ctrlEnable true;};
		} forEach LIFE_SETTINGS(getArray,"sendtoJail_locations");

		//rozwiaz
		_Btn10 ctrlsetText "rozwiaz";
		_Btn10 buttonSetAction "[] call life_fnc_untie; closeDialog 0;";
	};
	
	case civilian: {
		if (player getVariable ["isEscorting",false]) then {
			{ _x ctrlShow false; } forEach [_Btn1,_Btn2,_Btn4,_Btn5,_Btn6,_Btn7,_Btn8,_Btn9,_Btn10];
	...
	case independent: {
		...
		_Btn2 ctrlSetText "banadges";
		_Btn2 buttonSetAction "[life_pInact_curTarget,0] spawn life_fnc_threatAction; closeDialog 0;";
		if(life_inv_bandages < 1) then {
			_Btn2 ctrlEnable false;
		};

		_Btn3 ctrlSetText "morphine";
		_Btn3 buttonSetAction "[life_pInact_curTarget,1] spawn life_fnc_threatAction; closeDialog 0;";
		if(life_inv_morphine < 1) then {
			_Btn3 ctrlEnable false;
		};

		_Btn4 ctrlSetText "pawulon";
		_Btn4 buttonSetAction "[life_pInact_curTarget,2] spawn life_fnc_threatAction; closeDialog 0;";
		if(life_inv_pawulon < 1) then {
			_Btn4 ctrlEnable false;
		};
